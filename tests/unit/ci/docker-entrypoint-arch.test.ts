...
function hasPosixShell(): boolean {
  const probe = spawnSync("sh", ["-c", "true"], { stdio: "ignore" });
...
const describeWithShell = hasPosixShell() ? describe : describe.skip;

describeWithShell("docker-entrypoint CODEX_ARCH detection", () => {
