...
const BASH_SCRIPT = toBashPath(SCRIPT);
...
function run(env: Record<string, string>, timeoutMs = 10_000): RunResult {
  const bashEnv = {
    ...env,
    ...(env.RELEASE_DIR ? { RELEASE_DIR: toBashPath(env.RELEASE_DIR) } : {}),
  };
  const envPrefix = Object.entries(bashEnv)
    .map(([key, value]) => `${key}=${shellQuote(value)}`)
    .join(" ");
  const command = `${envPrefix} ${shellQuote(BASH_SCRIPT)}`.trim();
...
describe("electron-smoke.sh script", () => {
  ...
  it("is executable", () => {
    if (process.platform === "win32") {
      expect(readFileSync(SCRIPT, "utf-8")).toMatch(/^#!\/usr\/bin\/env bash/);
      return;
    }
  ...
  it("passes `bash -n` syntax check", () => {
    expect(() =>
      execFileSync("bash", ["-n", BASH_SCRIPT], { stdio: "pipe" }),
    ).not.toThrow();
  });
  ...
  it("fails loudly when RUNNER_OS is unset", () => {
    ...
    try {
      const out = execFileSync("bash", ["-lc", `unset RUNNER_OS; ${shellQuote(BASH_SCRIPT)}`], {
        env,
        encoding: "utf-8",
        stdio: ["ignore", "pipe", "pipe"],
      });
